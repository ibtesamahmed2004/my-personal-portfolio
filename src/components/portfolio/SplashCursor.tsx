...
// NOTE: This is the WebGL fluid cursor effect the user requested.
// Kept self-contained and tuned for smoothness (lower sim resolution on small screens).
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    const tunedSim = window.innerWidth < 768 ? Math.min(SIM_RESOLUTION, 72) : SIM_RESOLUTION;
    const tunedDye = window.innerWidth < 768 ? Math.min(DYE_RESOLUTION, 768) : DYE_RESOLUTION;

    const config = {
      SIM_RESOLUTION: tunedSim,
      DYE_RESOLUTION: tunedDye,
      CAPTURE_RESOLUTION,
      DENSITY_DISSIPATION,
    ...
    try {
      ...
      // WebGL unsupported or blocked (some low-end devices / privacy modes).
      // Fail gracefully: no cursor effect, no crash.
      return;
    }
    ...
    function getWebGLContext(c: HTMLCanvasElement) {
      ...
      let _gl = c.getContext("webgl2", params) as WebGL2RenderingContext | null;
      const isWebGL2 = !!_gl;
      if (!isWebGL2) {
        _gl =
          (c.getContext("webgl", params) as WebGL2RenderingContext | null) ||
      ...
      const halfFloatTexType = isWebGL2 ? (_gl as any).HALF_FLOAT : halfFloat && halfFloat.HALF_FLOAT_OES;
      ...
      if (isWebGL2) {
        formatRGBA = getSupportedFormat(_gl, (_gl as any).RGBA16F, _gl.RGBA, halfFloatTexType);
        formatRG = getSupportedFormat(_gl, (_gl as any).RG16F, (_gl as any).RG, halfFloatTexType);
        formatR = getSupportedFormat(_gl, (_gl as any).R16F, (_gl as any).RED, halfFloatTexType);
      } else {
      ...
      function getSupportedFormat(gl2: WebGL2RenderingContext, internalFormat: any, format: any, type: any) {
        if (!supportRenderTextureFormat(gl2, internalFormat, format, type)) {
          switch (internalFormat) {
            ...
            case (gl2 as any).RG16F:
              return getSupportedFormat(gl2, (gl2 as any).RGBA16F, gl2.RGBA, type);
            default:
              return null;
          }
        }
      ...
      function supportRenderTextureFormat(gl2: WebGL2RenderingContext, internalFormat: any, format: any, type: any) {
    ...
    function handleMouseDown(e: MouseEvent) {
      const pointer = pointers[0] as any;
